...
%%
% optimization
[A,b,Aeq,beq] = create_constraint_matrices(GSV,E,VC,coms,force,sC,sT,sB,'Bending',1);

objective = edge_lengths(GSV,E);
% objective = edge_lengths(GSV,E) + 1000*EAs.^2;
[x,ar,ax,be,fval] = optimize_lp(objective,A,b,Aeq,beq,'linprog','Bending',1);
% objective = lengths;
% objective(size(E,1)/2:end) = ...
%           objective(size(E,1)/2:end) + EAs(size(E,1)/2:end);

%%
save('rocker-arm.mat','AV','AF','ACV','ACF',...
