...
%       BT = mmread('../visibility-app/viewer/build/filename_b1.mtx');
%       A = mmread('../visibility-app/viewer/build/filename_a1.mtx');
%       b = mmread('../visibility-app/viewer/build/b_vec1.mtx');
% %       b = full(b);
%       objective = mmread('../visibility-app/viewer/build/obj_vec1.mtx');
% %       f = mmread('../visibility-app/viewer/build/forces_vec1.mtx');
%       f = mmread('../visibility-app/viewer/build/forces_bf1.mtx');
%       beq = full(f);
% %       Aeq = [sparse(size(BT,1),m) BT];
%       Aeq = BT;
% %       n = size(f,1)
%       m = size(BT,2)/2;
...
%     opt = mskoptimset(opt,'MSK_IPAR_INFEAS_REPORT_AUTO', 'MSK_ON');
    ...
    [an,~,flags,output] = linprog([obj;zeros(m*nf,1)], A, b,...
      ...
      opt);
%      [an,~,flags,output] = linprog(objective, A, b,...
%       Aeq, beq, ...
%       [zeros(m,1);-inf(m,1)], ...
%       [max_a*ones(m,1);inf(m,1)]);%, ...
%       %opt);
